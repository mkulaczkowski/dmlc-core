...
def submit(args):
    ...
    def mpi_submit(nworker, nserver, pass_envs):
        ...
        if nworker > 0:
            ...
            prog = 'mpirun --verbose --allow-run-as-root --display-map --display-devel-allocation --hostfile /kube-openmpi/generated/hostfile --map-by node -np %d %s %s' % (nworker, get_mpi_env(pass_envs), cmd)
        ...
        if nserver > 0:
            ...
            prog = 'mpirun --verbose --allow-run-as-root --display-map --display-devel-allocation --hostfile /kube-openmpi/generated/hostfile --map-by node -np %d %s %s' % (nserver, get_mpi_env(pass_envs), cmd)
            thread = Thread(target=run, args=(prog,))
            thread.setDaemon(True)
            thread.start()
